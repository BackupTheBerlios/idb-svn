...
$prequery = $safesql->query("select * from ".$Settings['sqltable']."topics where ID=%i", array($_GET['id']));
...
<tr class="TableRow1">
<td class="TableRow1" colspan="2"><span style="font-weight: bold; float: left;"><?php echo $SkinSet['TitleIcon'] ?><a href="Topic.php?act=View&amp;id=<?php echo $_GET['id']; ?>&amp;ForumID=<?php echo $_GET['ForumID']; ?>&amp;CatID=<?php echo $_GET['CatID']; ?>"><?php echo $TopicName; ?></a></span><?php if($SkinSet['TopicLayout']!="Type 2") { ?>
<span style="float: right;">&nbsp;</span><?php } ?></td>
</tr>
<?php
$query = $safesql->query("select * from ".$Settings['sqltable']."posts where TopicID=%i AND ForumID=%i and CategoryID=%i ORDER BY TimeStamp", array($_GET['id'],$_GET['ForumID'],$_GET['CatID']));
...
$requery = $safesql->query("select * from ".$Settings['sqltable']."members where ID=%i", array($MyUserID));
