...
function destroy( $id ) {
global $sqltable;
echo $sqld = query("DELETE FROM `".$sqltable."sessions` WHERE `session_id` = '$id'", array($id));
mysql_query($sqld);
return true; }
function gc() {
global $sqltable;
echo $sqlg = query('DELETE FROM `'.$sqltable.'sessions` WHERE `expires` < UNIX_TIMESTAMP();', array(null));
db_query($sqlg);
return true; }
